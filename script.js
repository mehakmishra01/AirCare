...
function updateAQIDisplayFromAQICN(aqiData) {
    ...
    aqiNote.textContent = 'Data from official monitoring stations (AQICN)';
...
async function fetchData(city) {
    ...
    try {
        ...
        // Fetch and display localities/monitoring stations (AQI.in style)
        try {
            const locations = await getCityLocations(coords.lat, coords.lon, coords.cityName || city);
            updateLocationsTable(locations, coords.cityName || city, weatherData);
        } catch (locErr) {
            console.warn('Locations fetch failed:', locErr);
            updateLocationsTable([], null, null);
        }
